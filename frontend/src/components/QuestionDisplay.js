import React, { useState, useRef, useEffecte } from 'react';
...
import { Cog6ToothIcon, TagIcon, XMarkIcon, ArrowLeftStartOnRectangleIcon } from '@heroicons/react/24/solid';
import ReactMarkdown from 'react-markdown';
import TestingConnectionBtn from './TestingConnectionBtn';
            ...
            <div className="mt-6 ml-2 rounded-lg">
                <div className='flex justify-between items-center'>
                    <div className="mb-2 flex items-center">
                        ...
                        {(() => {
                        const { bg, text, ring } = getComplexityColor(question.questionComplexity);
                        return (
                            <span className={`ml-4 inline-flex items-center rounded-full ${bg} px-2 py-1 text-xs font-medium ${text} ring-1 ring-inset ${ring}`}>
                    ...
                    <div className="flex text-white items-center h-12">
                        <div className="flex items-center gap-2 px-2 py-2 bg-red-500 text-white rounded cursor-pointer hover:bg-red-600 my-2"
                            onClick={() => navigate('/')}
                        >
