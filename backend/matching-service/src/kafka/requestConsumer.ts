    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }: EachMessagePayload) => {
        if (topic === 'matching-requests') {
          ...
          const matchResult = attemptMatch(matchRequest);
  
          if (matchResult) {
            // Send match result to 'matching-results' Kafka topic
            ...
            // No immediate match found; you might set up a retry mechanism or handle accordingly
          }
        }
      },
    });
